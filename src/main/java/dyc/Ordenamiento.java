...
public class Ordenamiento {
    public static void mergeSort(VectorTDA<Integer> s, int inicio, int fin) {
        if (inicio < fin) {
    ...
    private static void merge(VectorTDA<Integer> s, int inicio, int fin) {
        VectorTDA<Integer> r = new Vector<>();
        r.inicializarVector(fin - inicio + 1);
        int medio = (fin + inicio) / 2;
        int i = inicio;
        int j = medio + 1;
        for (int k = 0; k <= fin - inicio; k++) {
            if (j > fin || s.recuperarElemento(i) <= s.recuperarElemento(j) && i <= medio) {
